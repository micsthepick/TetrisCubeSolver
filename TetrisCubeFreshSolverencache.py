...
def emptyNeighbors(point):
    z, r, c = point
    global board, emtpyNeighCache
    try:
        for p in emptyNeighCache[point]:
            return p
    except KeyError:
        emptyNeighCache[point] = []
    if z > 0 and board[z-1][r][c] in EMPTYCHARS:
        newPoint = (z-1, r, c)
        emptyNeighCache[point].append(newPoint)
        yield newPoint
...
def checkSolvable(p):
    ...
    while len(toVisit) > 0:
        ...
        for singleP in singles:
            ...
            while True:
                seen1 = False
                for n in emptyNeighbors(singleP):
                    if n == point:
                        continue
                    if seen1:
                        break
